...
exports.createLinkstring = function(para){
	...
	var ls = '';
	for(var k in para){
//        if (k == "notify_id") para[k] = qs.unescape(para[k]);
		ls = ls + k + '=' + para[k] + '&';
	}
	ls = ls.substring(0, ls.length - 1);
	return ls;//qs.unescape(new Buffer(ls, 'utf8').toString());
}
...
exports.getHttpResponsePOST = function(url, cacert_url, para, input_charset, callback){
    ...
    var req = method(options, function(res) {
        var responseText='';
...
exports.getHttpResponseGET = function(url,cacert_url, callback){
    ...
    console.log("GetHttpResonseGET:Start");
    console.log("URL:host:" + options.hostname);
    console.log("URL:path:" + options.path);

    var req = method(options, function(res) {
        console.log("getHttpResponse Got callback");
