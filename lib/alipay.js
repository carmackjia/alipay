	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'TRADE_FINISHED'){                
				self.emit('create_direct_pay_by_user_trade_finished', out_trade_no, trade_no, {buyer_email: buyer_email, buyer_id: buyer_id});
			}
			else if(trade_status == 'TRADE_SUCCESS'){                
				self.emit('create_direct_pay_by_user_trade_success', out_trade_no, trade_no, {buyer_email: buyer_email, buyer_id: buyer_id});
			}
			res.send("success");		//请不要修改或删除
		}
		else {
...
Alipay.prototype.create_direct_pay_by_user_return = function(req, res){		
	...
	alipayNotify.verifyReturn(_GET, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'TRADE_FINISHED'){                
				return self.emit('create_direct_pay_by_user_trade_finished', res, out_trade_no, trade_no, {buyer_email: buyer_email, buyer_id: buyer_id});
			}
			else if(trade_status == 'TRADE_SUCCESS'){                
				return self.emit('create_direct_pay_by_user_trade_success', res, out_trade_no, trade_no, {buyer_email: buyer_email, buyer_id: buyer_id});
			}
