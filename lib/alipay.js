	...
	alipayNotify.verifyReturn(_GET, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'WAIT_BUYER_PAY'){                
				self.emit('trade_create_by_buyer_wait_buyer_pay_return', res, out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_SELLER_SEND_GOODS'){                
				self.emit('trade_create_by_buyer_wait_seller_send_goods_return', res, out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_BUYER_CONFIRM_GOODS'){                
				self.emit('trade_create_by_buyer_wait_buyer_confirm_goods_return', res, out_trade_no, trade_no);
			}
			else if(trade_status == 'TRADE_FINISHED'){                
				self.emit('trade_create_by_buyer_trade_finished_return', res, out_trade_no, trade_no);
			}
		}
		else{
			//验证失败
			self.emit("verify_fail_return", res);
		}
	});	
}
...
Alipay.prototype.trade_create_by_buyer_notify = function(req, res){
	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'WAIT_BUYER_PAY'){                
				self.emit('trade_create_by_buyer_wait_buyer_pay_notify', out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_SELLER_SEND_GOODS'){                
				self.emit('trade_create_by_buyer_wait_seller_send_goods_notify', out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_BUYER_CONFIRM_GOODS'){                
				self.emit('trade_create_by_buyer_wait_buyer_confirm_goods_notify', out_trade_no, trade_no);
			}
			else if(trade_status == 'TRADE_FINISHED'){                
				self.emit('trade_create_by_buyer_trade_finished_notify', out_trade_no, trade_no);
			}
...
Alipay.prototype.refund_fastpay_by_platform_pwd_notify = function(req, res){
	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			self.emit('refund_fastpay_by_platform_pwd_success_notify', batch_no, success_num, result_details);
...
Alipay.prototype.create_partner_trade_by_buyer_return = function(req, res){
	...
	alipayNotify.verifyReturn(_GET, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'WAIT_BUYER_PAY'){                
				self.emit('create_partner_trade_by_buyer_wait_buyer_pay_return', res, out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_SELLER_SEND_GOODS'){                
				self.emit('create_partner_trade_by_buyer_wait_seller_send_goods_return', res, out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_BUYER_CONFIRM_GOODS'){                
				self.emit('create_partner_trade_by_buyer_wait_buyer_confirm_goods_return', res, out_trade_no, trade_no);
			}
			else if(trade_status == 'TRADE_FINISHED'){                
				self.emit('create_partner_trade_by_buyer_trade_finished_return', res, out_trade_no, trade_no);
			}
		}
		else{
			//验证失败
...
Alipay.prototype.create_partner_trade_by_buyer_notify = function(req, res){
	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'WAIT_BUYER_PAY'){                
				self.emit('create_partner_trade_by_buyer_wait_buyer_pay_notify', out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_SELLER_SEND_GOODS'){                
				self.emit('create_partner_trade_by_buyer_wait_seller_send_goods_notify', out_trade_no, trade_no);
			}
			else if(trade_status == 'WAIT_BUYER_CONFIRM_GOODS'){                
				self.emit('create_partner_trade_by_buyer_wait_buyer_confirm_goods_notify', out_trade_no, trade_no);
			}
			else if(trade_status == 'TRADE_FINISHED'){                
				self.emit('create_partner_trade_by_buyer_trade_finished_notify', out_trade_no, trade_no);
			}
	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			var trade_status = _POST['trade_status'];
                        ...
                        var buyer_id = _POST['buyer_id'];

			if(trade_status  == 'TRADE_FINISHED' || trade_status == 'TRADE_SUCCESS'){
				self.emit('create_direct_pay_by_user_trade_success_notify', out_trade_no, trade_no, {
	...
	alipayNotify.verifyReturn(_GET, function(verify_result){
		if(verify_result) {//验证成功
			...
			var trade_status = _GET['trade_status'];
                        ...
                        //其他交易信息
                        var buyer_email = _GET['buyer_email'];
                        var buyer_id = _GET['buyer_id'];

			if(trade_status  == 'TRADE_FINISHED' || trade_status == 'TRADE_SUCCESS'){
				self.emit('create_direct_pay_by_user_trade_success_return', res, out_trade_no, trade_no, {
                    buyer_email: buyer_email,
                    buyer_id: buyer_id
                });
			}
